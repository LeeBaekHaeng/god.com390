		...
		if (isLocallyAuthenticated != null && (isLocallyAuthenticated.equals("true"))) {
			...
			if (isRemotelyAuthenticated == null) {
				...
				if (isSSOLoggedOn) {
					...
					httpResponse.sendRedirect(httpRequest.getContextPath() + "/sso.jsp");
//					RequestDispatcher dispatcher = httpRequest.getRequestDispatcher(httpRequest.getContextPath() + "/uat/uia/actionLogin.do");
...
//		if (requestURL.contains("/uat/uia/actionLogin.do")) {
			chain.doFilter(request, response);
//		} else {
//			httpResponse.sendRedirect(httpRequest.getContextPath() + "/EgovLoginUsr.jsp");
//		}
