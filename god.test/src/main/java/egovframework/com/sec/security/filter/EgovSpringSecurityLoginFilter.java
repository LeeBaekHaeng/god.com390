...
import god.test.menu.mapper.MenuMapper;
...
public class EgovSpringSecurityLoginFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		doFilterA1(request, httpResponse, chain, act, httpRequest);

		// 스프링 시큐리티 인증이 처리 되었는지 EgovUserDetailsHelper.getAuthenticatedUser() 메서드를 통해
		// 확인한다.
		// context-common.xml 빈 설정에 egovUserDetailsSecurityService를 등록 해서 사용해야 정상적으로
		// 동작한다.
			...
			if (isRemotelyAuthenticated != null && isRemotelyAuthenticated.equals("true")) {
				try {
					...
					if (loginVO != null && loginVO.getId() != null && !loginVO.getId().equals("")) {
						...
						// "/j_spring_security_check?j_username=" + loginVO.getUserSe() +
						// loginVO.getId() + "&j_password=" + loginVO.getUniqId());
						...
						// false 이면 chain 처리 되지 않음.. (filter가 아닌 경우 false로...)

						LOGGER.debug("before security filter call....");
						springSecurity.doFilter(new RequestWrapperForSecurity(httpRequest,
								loginVO.getUserSe() + loginVO.getId(), loginVO.getUniqId()), httpResponse, chain);
							...
							if (mapLockUserInfo != null) {
								...
								if (!sLoginIncorrectCode.equals("E")) {
									if (sLoginIncorrectCode.equals("L")) {
										request.setAttribute("message", egovMessageSource.getMessageArgs(
	...
	void doFilterA1(ServletRequest request, ServletResponse response, FilterChain chain, ApplicationContext act,
			HttpServletRequest httpRequest) {
		MenuMapper menuMapper = (MenuMapper) act.getBean("menuMapper");
		...
		MenuSiteMapVO vo = new MenuSiteMapVO();
...
//			}
		...
		httpRequest.setAttribute("GOD_MENUS", menuMapper.selectList(vo));
	}
