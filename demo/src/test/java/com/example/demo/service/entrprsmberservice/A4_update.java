...
class A4_update {
	...
	@Autowired
	public A4_update(EntrprsMberService service, EntrprsMberRepository repository) {
	...
	void test() {
		...
		dto.setEntrprsSeCode("C0000001");

		EntrprsMberDto update = service.update(dto);

		log.debug("update: {}", update);
		log.debug("getEntrprsMberId: {}", update.getEntrprsMberId());
		log.debug("getEntrprsSeCode: {}", update.getEntrprsSeCode());

		assertEquals(update.getEntrprsMberId(), dto.getEntrprsMberId());
		assertEquals(update.getEntrprsSeCode(), dto.getEntrprsSeCode());
	}
